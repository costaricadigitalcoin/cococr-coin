...
// which are part of the COCOCostaRica repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="cococostarica-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /cococostarica-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "cococostaricapay-cococostarica/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from cococostaricapay-cococostarica/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'cococostaricapay-cococostarica/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from cococostaricapay-cococostarica/develop"
            }
          }

          if (hasCache) {
            sh "cd /cococostarica-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /cococostarica-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /cococostarica-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /cococostarica-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /cococostarica-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /cococostarica-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /cococostarica-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }

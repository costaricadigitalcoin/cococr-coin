...
# cococostarica_hash
RUN git clone https://github.com/cococostaricapay/cococostarica_hash
RUN cd cococostarica_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} cococostarica
RUN useradd -u ${USER_ID} -g cococostarica -s /bin/bash -m -d /cococostarica cococostarica
...
RUN mkdir /cococostarica-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /cococostarica-src && \
  chown $USER_ID:$GROUP_ID /cache && \
  chown $USER_ID:$GROUP_ID /cache -R
WORKDIR /cococostarica-src

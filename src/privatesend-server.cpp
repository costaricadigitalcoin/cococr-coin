...
void CPrivateSendServer::ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if (fLiteMode) return; // ignore all COCOCostaRica related functionality
...
/*
    ...
    allow endless transaction that would bloat COCOCostaRica and make it unusable. To
...
{
    if (fLiteMode) return;        // disable all COCOCostaRica specific functionality

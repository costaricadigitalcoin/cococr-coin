...
const char * const BITCOIN_CONF_FILENAME = "cococostarica.conf";
const char * const BITCOIN_PID_FILENAME = "cococostaricad.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL || *cacheCounter != logAcceptCategoryCacheCounter.load())
        {
            ...
            if (mapMultiArgs.count("-debug")) {
                ...
                // "cococostarica" is a composite category enabling all COCOCostaRica-related debug output
                if(ptrCategory->count(std::string("cococostarica"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\COCOCostaRica
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\COCOCostaRica
    // Mac: ~/Library/Application Support/COCOCostaRica
    // Unix: ~/.cococostarica
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "COCOCostaRica";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/COCOCostaRica";
#else
    // Unix
    return pathRet / ".cococostarica";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty cococostarica.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override cococostarica.conf
